...
import { MagnifyingGlassIcon, PencilSquareIcon, CheckIcon, XMarkIcon, UserGroupIcon, BriefcaseIcon, PlusCircleIcon, StarIcon, SparklesIcon, ClockIcon, ClipboardDocumentListIcon, ArrowPathIcon } from '@heroicons/react/24/solid';
...
export const HomePage = () => {
  ...
  const [actionLoading, setActionLoading] = useState(null); // Loading cho từng nút hành động
  ...
  useEffect(() => {
    ...
    // 🔥 REALTIME: Vẫn giữ để nhận update từ người khác
  ...
  // --- HELPER ACTIONS (ĐÃ TỐI ƯU CẬP NHẬT NHANH) ---
  const handleSubmitRequest = async (e) => {
    ...
    try {
      ...
      // Reset form và cập nhật ngay
      setIsModalOpen(false); 
      setFormContent(''); setFormTime(''); setFormBounty('');
      alert(data.message); 
      await fetchRequests(); // 🔥 Cập nhật danh sách ngay lập tức
  ...
  const handleAccept = async (reqId) => {
    ...
    try {
      ...
      if (error) throw error;
      await fetchRequests(); // 🔥 Cập nhật ngay
  ...
  const handleComplete = async (reqId) => {
    ...
    try {
      ...
      await fetchRequests(); // 🔥 Xóa khỏi danh sách đang chờ
      await fetchHistory();  // 🔥 Thêm vào lịch sử ngay
      alert("Đã hoàn thành!");
    } catch (err) { alert("Lỗi: " + err.message); } finally { setActionLoading(null); }
  };
  ...
  const handleCancel = async (reqId) => {
    ...
    try {
      ...
      if (error) throw error;
      await fetchRequests(); // 🔥 Cập nhật ngay
      await fetchHistory();  // 🔥 Cập nhật lịch sử ngay
      alert("Đã hủy!");
    } catch (err) { alert("Lỗi: " + err.message); } finally { setActionLoading(null); }
  };
    ...
    <div className="bg-gray-900 text-white p-4 sm:p-8 rounded-lg shadow-2xl animate-fade-in font-sans">
      ...
      <div className="mb-12">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div className="bg-gray-800 p-6 rounded-lg shadow-lg border border-gray-700 hover:border-blue-500 transition-all duration-300"><div className="flex items-center mb-3"><Icon name="Swords" size={28} className="text-blue-400 mr-3" /><h3 className="text-2xl font-bold text-blue-400">🏕️ VN_game</h3></div><p className="text-gray-300 mb-2">Dành cho người chơi thích trải nghiệm ổn định, cày cuốc nhẹ nhàng.</p></div>
          <div className="bg-gray-800 p-6 rounded-lg shadow-lg border border-gray-700 hover:border-red-500 transition-all duration-300"><div className="flex items-center mb-3"><Icon name="Swords" size={28} className="text-red-400 mr-3" /><h3 className="text-2xl font-bold text-red-400">⚔️ VN_toichoi</h3></div><p className="text-gray-300 mb-2">Dành cho game thủ yêu thích PvP, chiến đấu sinh tồn khốc liệt.</p></div>
          <div className="bg-gray-800 p-6 rounded-lg shadow-lg border border-gray-700 hover:border-green-500 transition-all duration-300"><div className="flex items-center mb-3"><Icon name="Swords" size={28} className="text-green-400 mr-3" /><h3 className="text-2xl font-bold text-green-500">💖 VN_YenBinh</h3></div><p className="text-gray-300 mb-2">Thế giới "Yên Bình" dành cho các chiến binh hệ cute!</p></div>
          <div className="bg-gray-800 p-6 rounded-lg shadow-lg border border-gray-700 hover:border-yellow-500 transition-all duration-300"><div className="flex items-center mb-3"><Icon name="Swords" size={28} className="text-yellow-400 mr-3" /><h3 className="text-2xl font-bold text-yellow-400">⭐ Cổ Đông</h3></div><p className="mb-2 text-xl text-yellow-400">Admin: TQK (VN_game)</p><p className="text-gray-400 text-sm">Kêu gọi nhà đầu tư xây dựng server!</p></div>
        </div>
      </div>
      ...
      {/* =========== 🔥 TRUNG TÂM HỖ TRỢ (REALTIME & NO DELAY) 🔥 =========== */}
      <div className="mb-16 relative group">
        ...
        <div className="relative bg-gray-900 p-4 sm:p-8 rounded-2xl border border-yellow-500/20 shadow-2xl overflow-hidden">
            ...
            {/* Danh Sách Yêu Cầu (ĐÃ TỐI ƯU LOADING & REALTIME) */}
                     ...
                     <div className="mt-auto flex gap-2 justify-end">
                        ...
                        {user && user.id !== req.user_id && req.status === 'open' && (profile?.rank === 'helper' || profile?.role === 'admin') && (
                           <button 
                             ...
                             disabled={actionLoading === req.id} // Disable khi đang xử lý
      ...
      {/* Sự kiện & Slider (Giữ nguyên code cũ) */}
